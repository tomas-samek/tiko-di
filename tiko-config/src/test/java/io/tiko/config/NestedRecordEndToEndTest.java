...
class NestedRecordEndToEndTest {
    ...
    static TypeCoercer<DbConfig> dbCoercer() {
        return raw -> {
            ...
            int max = NestedRecordSupport.fieldOrDefault(node, "max", "DbConfig",
                Coercers.intCoercer(), 10);
    ...
    static TypeCoercer<FeatureFlag> flagCoercer() {
        return raw -> {
            ...
            boolean enabled = NestedRecordSupport.requireField(node, "enabled", "FeatureFlag",
                Coercers.booleanCoercer());
    ...
    static class AppConfigBinder implements ConfigBinder<AppConfig> {
        public Class<AppConfig> type() { return AppConfig.class; }
        public String prefix() { return "app"; }
        public AppConfig bind(Map<String, Object> root, BindContext ctx) {
            ...
            Optional<DbConfig> readReplica = ctx.optionalScalar(node, "readReplica",
                "app.readReplica", dbCoercer());
            List<Endpoint> endpoints = ctx.requireScalar(node, "endpoints", "app.endpoints",
                CompositeCoercers.list(endpointCoercer()), List.of());
            Map<String, FeatureFlag> flags = ctx.requireScalar(node, "flags", "app.flags",
                CompositeCoercers.map(flagCoercer()), Map.of());
    ...
    void all_four_nested_shapes_bind_from_yaml_like_map() {
        ...
        assertThat(cfg.endpoints()).containsExactly(
            new Endpoint("a.local", 8080),
            new Endpoint("b.local", 8081));
        assertThat(cfg.flags()).containsEntry("darkMode", new FeatureFlag(true))
