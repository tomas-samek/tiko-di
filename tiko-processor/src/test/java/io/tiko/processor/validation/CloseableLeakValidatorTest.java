...
class CloseableLeakValidatorTest {
    ...
    void warns_when_component_holds_closeable_field_without_cleanup() {
        Compilation compilation =
                Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(LEAKY_COMPONENT);

        assertThat(compilation).succeeded();
        assertThat(compilation).hadWarningContaining("holds AutoCloseable field 'stream'");
    }

    @Test
    void no_warning_when_component_implements_autocloseable_itself() {
        Compilation compilation =
                Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(SELF_AUTOCLOSEABLE_COMPONENT);
    ...
    void no_warning_when_component_declares_predestroy() {
        Compilation compilation =
                Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(PREDESTROY_COMPONENT);
    ...
    void suppression_at_field_level_silences_the_warning() {
        Compilation compilation =
                Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(SUPPRESSED_COMPONENT);
    ...
    private static final JavaFileObject SELF_AUTOCLOSEABLE_COMPONENT = JavaFileObjects.forSourceLines(
    ...
    private static final JavaFileObject PREDESTROY_COMPONENT = JavaFileObjects.forSourceLines(
    ...
    private static final JavaFileObject SUPPRESSED_COMPONENT = JavaFileObjects.forSourceLines(
    ...
    private static final JavaFileObject AUTOCLOSEABLE_DEP = JavaFileObjects.forSourceLines(
    ...
    private static final JavaFileObject INJECTED_DEPENDENCY_COMPONENT = JavaFileObjects.forSourceLines(
