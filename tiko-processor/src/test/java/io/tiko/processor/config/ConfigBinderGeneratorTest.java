...
class ConfigBinderGeneratorTest {
    ...
    void direct_nested_record_generates_NestedCoercer_and_references_it() throws IOException {
        ...
        JavaFileObject inner = JavaFileObjects.forSourceLines(
            "io.example.DbConfig",
            "package io.example;",
            "public record DbConfig(String url, int max) {}"
        );
    ...
    void map_of_nested_records_composes_with_CompositeCoercers_map() throws IOException {
        ...
        JavaFileObject inner = JavaFileObjects.forSourceLines(
            "io.example.Flag",
            "package io.example;",
            "public record Flag(boolean enabled) {}"
        );
        Compilation c = Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(outer, inner);
        com.google.testing.compile.CompilationSubject.assertThat(c).succeeded();
    ...
    void deeply_nested_records_emit_cascaded_coercers() throws IOException {
        ...
        Compilation c = Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(outer, mid, inner);
        com.google.testing.compile.CompilationSubject.assertThat(c).succeeded();

        boolean hasOuterCoercer = c.generatedSourceFiles().stream()
            .anyMatch(f -> f.getName().contains("OuterNestedCoercer_"));
    ...
    void same_nested_record_used_in_two_configs_emits_one_coercer() throws IOException {
        ...
        JavaFileObject shared = JavaFileObjects.forSourceLines(
            "io.example.Shared",
            "package io.example;",
            "public record Shared(int v) {}"
        );
        Compilation c = Compiler.javac().withProcessors(new TikoAnnotationProcessor()).compile(cfgA, cfgB, shared);
        com.google.testing.compile.CompilationSubject.assertThat(c).succeeded();
